...
import {
  ...
  clearActiveJob, pollJobStatus, STAGE_PROGRESS,
} from '../store/slices/detectionSlice';
import { getSocket, initSocket } from '../services/socket';
import { Upload, ScanLine, Settings, X, CheckCircle, AlertCircle, Loader, Image, Zap, Eye } from 'lucide-react';
import VehicleDetailsPanel from '../components/VehicleDetailsPanel';
import styles from './DetectionPage.module.css';

const POLL_INTERVAL_MS  = 2500;   // fallback HTTP poll every 2.5 s
const SOCKET_TIMEOUT_MS = 4000;   // wait 4 s for socket before switching to poll-only
...
export default function DetectionPage() {
  ...
  useEffect(() => {
    ...
    // Ensure socket is initialised and authenticated
    const sock = initSocket(user?._id);
    socketReady.current = false;

    // Give socket SOCKET_TIMEOUT_MS to connect; if it doesn't, polling handles it
    ...
    // Always start polling as fallback — stops itself when job finishes
    startPolling(jobId);
  ...
  const currentStageIdx = processingStages.findIndex(s => s.key === jobStatus);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>Detect License Plate</h1>
          <p className={styles.subtitle}>
            Upload an image to detect and extract vehicle number plates
          </p>
        </div>
        <button className="btn btn-ghost" onClick={() => setShowOptions(!showOptions)}>
          <Settings size={15} /> Options
        </button>
      </div>
          ...
          {!preview ? (
            ...
            >
              ...
              <div className={styles.dropzoneContent}>
                ...
                <h3>{isDragActive ? 'Drop it here!' : 'Upload Image'}</h3>
                <p>Drag & drop or <span>click to browse</span></p>
                <div className={styles.dropzoneFormats}>
                  {['JPEG','PNG','WebP','BMP','GIF'].map(f => <span key={f}>{f}</span>)}
                </div>
                <p className={styles.dropzoneLimit}>Max 10 MB</p>
              </div>
            </div>
          ) : (
            <div className={styles.previewSection}>
              ...
              <div className={styles.imagePreview}>
                  ...
                  <div className={styles.scanOverlay}>
                    ...
                    <div className={styles.scanCorner}
                      style={{ top: 10, right: 10,
                        borderRight: '2px solid var(--accent)', borderLeft: 'none' }} />
                    <div className={styles.scanCorner}
                      style={{ bottom: 10, left: 10,
                        borderBottom: '2px solid var(--accent)', borderTop: 'none' }} />
                    <div className={styles.scanCorner}
                ...
                {result?.processedImages?.find(i => i.type === 'annotated') && (
                  <div className={styles.imageToggle}>
                    <button onClick={() => {
                      const annotatedUrl =
                        `${API_BASE}${result.processedImages.find(i => i.type === 'annotated').url}`;
                      setPreview(prev =>
                        prev === previewUrl.current ? annotatedUrl : previewUrl.current
                      );
                    }}>
                      <Eye size={13} />
            ...
            <div className={styles.results}>
              ...
              {result.processedImages?.length > 0 && (
                <div className={styles.processedImages}>
                  <h4>Processed Versions</h4>
                  <div className={styles.processedGrid}>
                    {result.processedImages.map((img, i) => (
                      <a
                        key={i}
                        href={`${API_BASE}${img.url}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className={styles.processedThumb}
                      >
